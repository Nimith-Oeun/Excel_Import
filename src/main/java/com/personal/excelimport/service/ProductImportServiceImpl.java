...
public class ProductImportServiceImpl implements ProductImportService {
    ...
    public Map<Integer, String> importProduct(MultipartFile file) {
        ...
        try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
            ...
            if (sheet == null) {
                errors.put(0, "No sheet found in the Excel file.");
                return errors;
            }
            ...
            for (Row row : sheet) {
                ...
                try {
                    ...
                    menuItem.setCreatedAt(LocalDateTime.now().withNano(0));
                    menuItem.setUpdatedAt(LocalDateTime.now().withNano(0));

                    productImportRepository.save(menuItem);

                } catch (Exception e) {
                    errors.put(rowNum, "Row " + rowNum + ": " + e.getMessage());
                }
            }
...
//        Pageable secondPageWithFiveElements = PageRequest.of(1, size);

        return productImportRepository.findAllByPrice(3,pageRequest); // pagination with filter price = 3
